...
def build_instruction(goal: str, last_action: str | None) -> str:
    ...
    return (
        "你是一个安卓游戏自动操作智能体，正在通过截图观察游戏画面并决定下一步操作。\n"
        f"当前任务目标：{goal}。\n"
        f"{history}"
        "画面坐标以百分比表示：x 为横向 0.0（最左）到 1.0（最右），"
        "y 为纵向 0.0（最上）到 1.0（最下）。\n"
        "观察画面后只输出一个 JSON 对象，不要输出任何其他文字：\n"
        '点击画面上的某个按钮或可交互元素：{"action":"tap","x":0.59,"y":0.64}\n'
        '（x、y 为该元素中心的百分比坐标，必须在 0~1 之间）\n'
        '画面正在加载、播放动画或没有合适目标时：{"action":"wait"}\n'
        "规则：只输出 JSON；不要编造画面中不存在的元素；优先选择能推进任务目标的按钮。\n"
    )
